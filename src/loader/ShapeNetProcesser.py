...
class ShapeNetProcesser(LoaderInterface):
    ...
    def run(self):
        used_shapenet_objs = random.choices(self._files_used, k=self._num_objects)
        for i, selected_obj_info in enumerate(used_shapenet_objs):
            ...
            for obj in loaded_obj:
                ...
                for j in range(len(obj.material_slots)):
                    ...
                    cc_mat = None
                    while cc_mat is None:
                        mat_folder_name = random.choice(self._cc_assets)
